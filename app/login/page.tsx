...
function LoginInner() {
  ...
  // The QuickCart mobile app (a WebView wrapper) sets this flag on every
  // page load. Google's device-account picker never appears inside a
  // WebView, so in-app users go straight to Email OTP instead of a Google
  // button that would only offer a manual email/password form.
  ...
  useEffect(() => {
    ...
    if (window.localStorage.getItem("quickcart_is_app") === "1") {
      setIsApp(true);
  ...
  return (
    <main className="relative flex min-h-screen w-full flex-col justify-between overflow-x-hidden overflow-y-auto bg-slate-950 font-sans antialiased">
      {/* Hero Section with Arch Ellipse Clip taking ~58% of viewport */}
      <div
        className="relative h-[48vh] sm:h-[55vh] w-full shrink-0 overflow-hidden"
      ...
      <div className="relative z-10 -mt-10 flex flex-1 flex-col items-center justify-between rounded-t-3xl border-t border-white/20 bg-slate-900/95 px-6 pb-6 pt-6 shadow-2xl backdrop-blur-2xl text-white">
        ...
        {error && !showEmailInput && (
          <div className="w-full max-w-sm mt-3 rounded-xl border border-red-500/50 bg-red-950/60 p-2.5 text-center text-xs font-semibold text-red-300">
            {error}
          </div>
        )}
        ...
        <div className="w-full max-w-sm space-y-2.5 my-auto pt-3">
          ...
          {!isApp && (
            <button
              ...
              className="group flex w-full items-center justify-center gap-3 rounded-full border border-slate-700 bg-white py-3.5 text-sm font-bold text-slate-900 shadow-md transition-all duration-200 hover:bg-slate-100 disabled:opacity-60 active:scale-95"
            >
              {googleLoading ? (
